...
class RCSender {
    constructor(pin) {
        ...
        if (pin < 1)
            pin = 1;
        child_process_1.execSync(`chmod +rwx ${__dirname}/rc-sender-linux-arm32`);
    }
    /**
    ...
    send(code, length) {
        return __awaiter(this, void 0, void 0, function* () {
            return new Promise((promise) => {
                child_process_1.exec(`${__dirname}/rc-sender-linux-arm32 ${this._pin} ${code} ${length} ${this._protocol} ${this._pulseLength} ${this._repeatTransmit}`, (error, stdout, stderr) => {
                    if (error != null) {
